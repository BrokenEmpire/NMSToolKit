...
{
	/****************************************************************************************************/
	PickableApplication::PickableApplication(const std::string& title, const Arguments& arguments, size_t indexDataSize, const Vector2i& defaultWindowSize) :
		ImGuiApplication { title, arguments, defaultWindowSize }
	{
		...
		m_FrameBuffer.attachRenderbuffer(GL::Framebuffer::ColorAttachment { 0 }, m_RBColor)
			...
			.mapForDraw({ { Shaders::Phong::ColorOutput, GL::Framebuffer::ColorAttachment { 0 } }, { Shaders::Phong::ObjectIdOutput, GL::Framebuffer::ColorAttachment { 1 } } });
